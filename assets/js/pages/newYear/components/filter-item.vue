  ...
  <div class="filterItem">
    ...
    <div class="filterItem__list" v-if="allCheck.length > 0 && !isLoading">
      <div class="list__container" ref="list">
        <label for="oneLabel" class="list__item" v-if="textOneInput !== ''">
          <input type="checkbox" name="oneLabel" id="oneLabel" value="all" v-model="valueChecks"
            @change="clearAll()" />
          ...
          <div class="item__text">{{ `${textOneInput}` }}</div>
        </label>
        <label :for="item.ID" class="list__item" v-for="item in viewCheck()" :key="item.ID">
          <input type="checkbox" :name="item.title" :id="item.ID" :value="item.ID" v-model="valueChecks"
      ...
      <button class="list__viewAll" ref="btnViewAll" v-if="viewCheck().length > viewItem" @click="isOpen = !isOpen">
